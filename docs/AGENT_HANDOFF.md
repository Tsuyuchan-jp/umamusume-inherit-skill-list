...
1. **スキルの手動除外** ← 次
2. リストに獲得バ身[バ]とポイント効率[バ/Pt]を併記
3. コース選択を U-tools の [コース一覧](https://xn--gck1f423k.xn--1bvt37a.tools/race/tracks) に近い UI へ
4. 背景を sp-calc と差別化

1の合意: 行ごとに外せる。コピーは残したスキルの先頭25件。除外は編成と一緒にこのブラウザへ残す。
